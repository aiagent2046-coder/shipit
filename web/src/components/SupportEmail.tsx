 ...
 * lost, a PayPal order that didn't save, and a confirmed Pro transfer. Those
 * are the moments a stranger has already paid and has nothing, which is
 * exactly when a dead end turns a refund conversation into an accusation of
 * fraud.
 *
 * Deliberately just the link: each caller keeps its own sentence and its own
 * identifier (invoice id, order id, reference, payer name and amount), because
 * what the operator needs to match on differs per payment channel.
 *
