...
function score(reason = "check_error: RuntimeError"): Score {
  return { total: 10, categories: { Security: 10 }, basis: "static+llm",
    ...
    scan_manifest: {
      ...
      static_checks_not_run: [{ check: "session_cookie", reason }],
...
it("renders a failed check despite a completed model review and an empty finding list", () => {
  render(<AuditCoverage score={score()} findings={[]} />);
  const notice = screen.getByRole("complementary", { name: "Static checks failed" });
  expect(notice.textContent).toContain("session_cookie: check_error: RuntimeError");
  expect(notice.textContent).toContain("cannot be used for comparison");
  expect(screen.getByText("Static check failed: session_cookie")).toBeTruthy();
