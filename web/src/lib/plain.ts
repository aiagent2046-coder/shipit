...
const PLAIN: Record<string, { what: string; risk: string; fix: string }> = {
  ...
  "unsafe-xml-parse": {"what": "XML is parsed with external entity resolution explicitly enabled.", "risk": "lxml.etree resolves external entities when resolve_entities=True is passed. An attacker-controlled XML document can then read local files, probe the internal network or force a billion-laughs expansion. Where the bytes come from has not been verified.", "fix": "Parse XML with xml.etree.ElementTree (which does not resolve external entities) or the defusedxml package. For lxml, drop resolve_entities=True (the default is 'internal' on lxml 5.0+/6.1.0+, which does not resolve external entities) or pass resolve_entities=False."},
