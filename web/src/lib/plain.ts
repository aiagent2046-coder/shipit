...
const PLAIN: Record<string, { what: string; risk: string; fix: string }> = {
  ...
  "unsafe-xml-parse": {"what": "XML is parsed through a path that can resolve external entities.", "risk": "lxml.etree resolves external entities when resolve_entities=True is passed, and does so by default before lxml 5.0 (from 5.0 the default is 'internal', which does not). An attacker-controlled XML document can then read local files, probe the internal network or force a billion-laughs expansion. Where the bytes come from has not been verified.", "fix": "Parse XML with xml.etree.ElementTree (which does not resolve external entities) or the defusedxml package. For lxml, pass resolve_entities=False (or parser=etree.XMLParser(resolve_entities=False)) and disable network access."},
