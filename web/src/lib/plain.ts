...
const PLAIN: Record<string, { what: string; risk: string; fix: string }> = {
  "unsafe-xml-parse": {"what": "XML is parsed without protection against external entities.", "risk": "lxml and the stdlib minidom/sax/pulldom resolve external entities by default, so an attacker-controlled XML document can read local files, probe the internal network or force a billion-laughs expansion. Where the bytes come from has not been verified; trusted internal data and untrusted external data can reach the same call.", "fix": "Parse XML with xml.etree.ElementTree (which does not resolve external entities) or the defusedxml package. For lxml, pass parser=etree.XMLParser(resolve_entities=False) and disable network access. Never parse untrusted XML with minidom, sax or pulldom."},
