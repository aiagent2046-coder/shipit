...
const PLAIN: Record<string, { what: string; risk: string; fix: string }> = {
  "unsafe-deserialization": {"what": "Data is turned back into objects through a format that can run code.", "risk": "This call loads data through a format that can name classes and call them while it loads, so bytes chosen by somebody else run with this process's privileges. Where the bytes come from has not been checked -- a payload this same process wrote is ordinary code, and the call looks identical either way.", "fix": "Use a format that carries values rather than objects (json, msgpack, cbor) with an explicit schema. Where a pickle-based format is unavoidable, check a signature or checksum over the payload first, and never load data that arrived in a request, an upload or a message from another system."},
