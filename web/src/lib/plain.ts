...
const PLAIN: Record<string, { what: string; risk: string; fix: string }> = {
  "archive-extraction-fully-trusted": {"what": "Archive extraction explicitly disables safety filtering.", "risk": "tarfile extract or extractall is called with filter=\"fully_trusted\", the explicit opt-out from extraction checks. A member named with an absolute path or ../ components then writes outside the destination on every Python with the parameter (measured on 3.12.13: fully_trusted writes outside where the data and tar filters refuse the same member). Where the archive bytes come from has not been verified.", "fix": "Drop filter=\"fully_trusted\": pass filter=\"data\" (the 3.14 default) or filter=\"tar\", or omit the argument on Python 3.14+. Never extract untrusted archives with filtering disabled."},
  "command-injection-shell-built-command": {"what": "A shell command is built from a value the caller sent.", "risk": "This route hands a string to a shell -- os.system, os.popen, subprocess with shell=True, or an explicit POSIX shell -c -- and that string is assembled from the request itself. A value carrying shell characters (;, &&, $(...), backticks) can run a second command the application never wrote, up to arbitrary code as the process user. Whether the route is reachable, whether anything outside this function constrains the value, and whether the call actually runs have not been verified.", "fix": "Pass request values as separate arguments to the intended executable, without shell=True or an explicit shell -c. In a necessary POSIX shell script, keep the script fixed and pass values as positional parameters with quoted expansions. shlex.quote applies to POSIX shells, not Windows cmd.exe; validate command and option choices separately."},
  "insecure-randomness": {"what": "A secret-looking value is generated from a non-cryptographic random source.", "risk": "A token, password, reset link, OTP or nonce is drawn from Math.random or Python's random module. That source is predictable, so the value can be guessed or replayed -- the classic account-takeover vector. Whether the value is actually used as a secret and whether anything else re-randomizes it have not been verified.", "fix": "Use a cryptographically secure source: crypto.getRandomValues in JS/TS, or Python's secrets module (secrets.token_hex / secrets.token_urlsafe). Never derive a token, password, reset link, OTP, nonce or salt from Math.random or the random module."},
  "python-open-redirect-unvalidated-url": {"what": "A redirect targets a URL whose authority comes from the caller.", "risk": "A redirect constructor sends the visitor's browser to an address whose host is built from the request itself. A caller who controls the target can redirect a signed-in user to an attacker's domain -- the open redirect that carries OAuth codes, session tokens in the Referer and phishing flows. Whether the route is reachable and whether anything outside this function constrains the target have not been verified.", "fix": "Validate the target before redirecting: allow only relative paths, or compare the resolved host against an allowlist of your own origins and reject everything else (including protocol-relative //host and backslashes that some parsers read as //host). Never redirect straight to a value the request supplied."},
  "xss-unsafe-html-injection": {"what": "HTML is injected into the DOM from a value that is not a fixed string.", "risk": "A non-literal value reaches an HTML-injection sink -- dangerouslySetInnerHTML, innerHTML/outerHTML, document.write or insertAdjacentHTML. If that value can carry attacker-controlled text, it injects markup and script into the page: script execution under the visitor's origin, token theft and DOM corruption. Whether the value was sanitized, whether it is reachable, and whether the sink runs have not been verified.", "fix": "Use textContent (or React children) for anything that is text, not markup. If HTML must be inserted, sanitize the value first (DOMPurify with an allowlist, or an equivalent) and avoid building HTML from strings. In React, avoid dangerouslySetInnerHTML unless the content is already trusted."},
  "path-traversal-file-sink": {"what": "A file path is built from a value the caller sent.", "risk": "This route hands a filesystem path to a file operation, and that path is assembled from the request itself. A value like ../../etc/passwd, or a name ending in a served extension, can make the application read or write files it never meant to touch. Whether anything outside this function constrains the path has not been checked.", "fix": "Keep only a name, not a path: pass the value through secure_filename (or whitelist its characters), join it to a fixed base directory, then confirm the result is still inside that base with resolve() and is_relative_to(). Never serve a file by a path that came straight from the request."},
  "insecure-session-cookie-attributes": {"what": "A session cookie is set without the protection the browser offers.", "risk": "The cookie that carries the session is created without HttpOnly, which keeps it out of reach of scripts, or with SameSite=None, which tells the browser to send it on cross-site requests too. Either way a flaw elsewhere -- a script that reaches the page, or a form on another site -- can use the session. Whether the value is a live session token, whether another layer rewrites the cookie, and whether the request is reachable have not been verified.", "fix": "Set HttpOnly on the session cookie (httponly=True in Python, httpOnly: true in TypeScript) and leave SameSite at its default or set it to Lax/Strict. Use SameSite=None only when the cookie must cross sites, keep Secure on, and verify the request origin wherever the cookie authorizes a change."},
  "unsafe-deserialization": {"what": "A deserialization call requires trusted input or an explicit loader.", "risk": "Pickle-family and confirmed unsafe YAML loaders can invoke code while reconstructing objects. Marshal reconstructs values or code objects without executing those objects; YAML without Loader depends on the installed version and raises TypeError in modern PyYAML. The finding describes which case was observed. Input trust and runtime exploitability have not been verified.", "fix": "Prefer json (or msgpack/cbor) with an explicit schema, or yaml.safe_load for YAML values. If object serialization is unavoidable, accept only trusted producers and verify an HMAC or digital signature before loading. A checksum supplied with untrusted bytes does not authenticate them; an expected digest must come from a trusted source."},
  "unsafe-xml-parse": {"what": "An XML parse explicitly enables entity resolution.", "risk": "If the XML is untrusted, resolve_entities=True can allow external entity access. Input trust, custom resolvers, runtime parser behavior and network controls have not been verified.", "fix": "For lxml.etree.parse/fromstring, pass parser=etree.XMLParser(resolve_entities=False, no_network=True). For lxml.etree.iterparse, pass resolve_entities=False, no_network=True directly. Verify whether the application needs entity expansion before changing parsing behavior."},
  "tls-verification-disabled": {"what": "TLS certificate or hostname verification is weakened.", "risk": "A recognised client/context setting weakens peer-identity checks. Disabling hostname matching can leave certificate-chain validation enabled; disabling chain validation is a different setting. The finding identifies the control seen in source. Runtime use and exposure have not been verified.", "fix": "Restore the affected check: require a trusted certificate chain and match the expected hostname. Configure the client's supported CA/context option for private certificates instead of disabling verification."},
  "python-outbound-request-unvalidated-url": {"what": "An HTTP client receives an address derived from request input.", "risk": "A caller-controlled value reaches the host portion of a request target or client configuration without a preceding local address check recognized by the scanner. If external controls do not restrict it, a request may reach unintended services. Runtime requests, network reachability and checks outside this handler have not been verified.", "fix": "Allow only approved schemes and hosts. Check resolved addresses against the permitted destinations, including private and link-local ranges, and ensure the connection uses the address that was checked. Disable redirects or validate every redirect destination too."},
  "python-route-write-auth-consistency": {"what": "A route contains a call recognized as a write and shows no local identity check.", "risk": "A sibling write route on the same router declares an identity check. This is a local consistency signal based on calls and dependency names; actual changes to stored data, middleware, router mounting and runtime access have not been verified.", "fix": "Confirm whether this route is meant to be reachable without an identity -- a webhook or an onboarding step would be. If not, apply the sibling route's identity contract, then test a write with a foreign or absent credential against synthetic records."},
