# Drydock — техническая архитектура MVP
...
1. **Максимальное переиспользование Palantir-офиса.** Оркестратор агентов, фаза блокирующего ревью (viktor), read-before-write, мульти-провайдерный слой — уже написаны и покрыты 83 тестами. Drydock — это новый «клиент» этого движка, а не новая система.
2. **Детерминизм прежде LLM.** Всё, что можно проверить статическим анализатором, проверяется статическим анализатором. LLM подключается только там, где нужна семантика. Когда LLM всё же нужен и какой именно (облачный для финальных вердиктов, локальный для bulk/приватного), см. [llm-stage-provider-policy.md](llm-stage-provider-policy.md).
...
> **Примечание v0.3 (2026-07-13, по результатам батч-прогона на 10 реальных
> vibe-coded репо).** Gate воспроизводимости закрыт со следующими измеренными
> фактами и продуктовым решением:
> 1. Статический скан детерминирован полностью (спред 0.0).
> 2. Скор (v2: категорийные полы + взвешенное среднее) — спред ≤0.6 между
>    LLM-прогонами, последние пары 0.0–0.2.
> 3. Critical-находки LLM-стадии воспроизводимы на 100%; high — семантически
>    стабильны, координатно ~50–70%: модель недетерминирована даже при
>    temperature=0, и на насыщенном репо (истинных проблем больше капа на
>    промпт) одиночный проход — это сэмпл. Инструментально не лечится.
> 4. Решение: **бесплатный аудит = 1 проход** (скор и critical стабильны —
>    этого достаточно для шарабельного отчёта); **платный Fix Pack =
>    union-of-2 прохода** (`run_llm_scan(..., passes=2)`) — полнота скоупа
>    работ, удвоенная LLM-себестоимость (~350₽) покрывается ценой Pack.
> Также по итогам батча: правило secrets-in-migrations (системный паттерн
> Lovable-экспортов), doc-context демпфер, кросс-рубричный дедуп, ретраи
> транзиентных 5xx, score.basis.
