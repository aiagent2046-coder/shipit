...
# keys.env holds the stand's two JWTs and is generated by the e2e, never
# committed, so shellcheck cannot follow it. The directive has to sit directly
# above `source`: it binds to the NEXT COMMAND, and when these were one
# semicolon-separated line it bound to `set -a` and suppressed nothing.
set -a
