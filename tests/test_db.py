...
class TestPaymentRepositoryWithFakePool:
    async def test_create_inserts_with_correct_param_order(self, monkeypatch):
        ...
        # The five trailing Nones are payer_name, payer_email (migration
        # 0026), payer_x (0032), payer_locale (0033) and provider_payment_id
        # (0034): each is supplied by one rail and left alone by the others, so
        # every provider writes the row exactly as it did before those columns
        # existed. The one before them is audit_id.
        #
        # provider_payment_id is the payment's identity in the system that took
        # the money -- ЮKassa's id, not ours. It is last on purpose: a column
        # inserted into the middle of this list renames every parameter after
        # it, and this assertion is the only thing that would notice.
        #
