...
# --- commented-out SQL is not SQL -------------------------------------------
#
# MEASURED in the same survey, in a repository whose exported schema opened
# with `-- CREATE TABLE STATEMENT (generated)`. It produced a table named
# `statement`, and the comment's own `(` then opened a body that ran to the
# next `);` — swallowing the real CREATE TABLE underneath it, so a table the
# customer actually has vanished from the schema entirely.

def test_a_comment_does_not_declare_a_table() -> None:
    schema = parse_schema("""
        -- CREATE TABLE STATEMENT (generated by the export tool)
        create table public.real_one (id uuid primary key, email text);
        alter table public.real_one enable row level security;
    """)
    assert sorted(schema) == ["real_one"]
    assert schema["real_one"].rls_enabled is True


def test_a_commented_out_enable_is_not_protection() -> None:
    """The quiet direction, and the one that loses a customer real money: a
    line the developer disabled counted as the thing being switched on, so a
    live hole would never have been reported."""
    schema = parse_schema("""
        create table public.leaky (id uuid primary key, email text);
        -- alter table public.leaky enable row level security;
    """)
    assert schema["leaky"].rls_enabled is False


def test_a_commented_out_policy_is_not_a_hole() -> None:
    schema = parse_schema("""
        create table public.t (id uuid primary key, email text);
        alter table public.t enable row level security;
...
def test_a_policy_inside_a_do_block_is_still_seen() -> None:
    """Dollar-quoted bodies are passed over, not dropped. Supabase migrations
    routinely wrap real policies in `DO $$ … END $$;`, and hiding those would
    make protection that exists invisible — a table would read as open."""
    schema = parse_schema("""
...
def test_a_double_dash_inside_a_string_is_not_a_comment() -> None:
    schema = parse_schema("""
        create table public.t (id uuid primary key, note text default 'a -- b');
        alter table public.t enable row level security;
    """)
    assert schema["t"].rls_enabled is True
    assert schema["t"].columns == ["id", "note"]


def test_a_backslash_does_not_escape_the_closing_quote() -> None:
    r"""Postgres ships `standard_conforming_strings = on`, so `'C:\'` is a
    complete string. Reading the backslash as an escape leaves the scanner
    believing it is still inside a literal, and every comment after it stops
    being stripped.

    The commented-out ENABLE below is what makes that observable. A first
    version of this test asserted on a plain ALTER and passed either way — the
    mutation that treats backslash as an escape survived it, because being
    "inside a string" does not stop the regex from matching text nobody
    blanked.
    """
    schema = parse_schema("""
        create table public.t (id uuid primary key, path text default 'C:\\');
        -- alter table public.t enable row level security;
    """)
    assert schema["t"].rls_enabled is False
