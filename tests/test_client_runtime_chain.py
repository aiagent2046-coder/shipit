...
def base():
    return {
        ...
        "observations": [],
        "budget": {},
        "source": {"archive_sha256": ARCHIVE_SHA256, "engine_version": "test"},
    }
...
def test_bound_import_chain_and_saved_roundtrip():
    ...
    chain = agent["client_runtime_chain"]
    previous = None
...
@pytest.mark.parametrize(
    "field,value", [("input_sha256", "0" * 64), ("attempts", True), ("reason", "executed_customer_code")]
)
...
def test_pipeline_import_is_no_llm_and_does_not_change_findings(monkeypatch):
    ...
    result = run_scan(
        ...
        client_runtime_run_id=evidence["run_id"],
    )
    ...
    rejected = run_scan(
        ...
        depth=BASIS_PREVIEW,
        client_runtime_evidence=evidence,
        client_runtime_run_id="wrong-run",
    )
...
def test_cli_rejects_duplicate_keys_and_existing_output(tmp_path):
    ...
    zip_path.write_bytes(archive({"README.md": "hello"}))
    evidence = tmp_path / "scenario.json"
    evidence.write_text('{"status":"failed","status":"passed"}')
