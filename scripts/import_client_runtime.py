...
from app.ingest.validators import MAX_ARCHIVE_BYTES, ArchiveValidationError
...
def main(argv=None):
    ...
    parser.add_argument('--output', type=Path, required=True, help='New report JSON; existing files are not overwritten')
    ...
    try:
        ...
        with args.output.open('x', encoding='utf-8') as output:
            ...
            evidence = json.loads(payload, object_pairs_hook=unique_object,
            ...
            result = run_scan(raw, LLMClient(providers=[]), depth=BASIS_PREVIEW,
                              client_runtime_evidence=evidence, client_runtime_run_id=args.run_id)
        ...
        print(json.dumps({'status': 'accepted' if accepted else 'rejected',
                          'scope': 'operator_evidence_consistency',
                          'model_calls': result['score']['scan_manifest']['model_calls'],
                          'customer_project_verified': False, 'automatic_patch': False}))
